test_that("Output class is as expected.", {
  ...
  expect_s3_class(get_centroid(c(367773, 5703579), epsg = 25832), c("sfc_POINT", "sfc"))
})

test_that("Function working as intended.", {

  p <- get_centroid(c(6.09, 50.46), epsg = 4326) |> sf::st_coordinates() |> as.numeric()
  expect_equal(p, c(6.09, 50.46))

  p <- get_centroid(c(367773, 5703579), epsg = 25832) |> sf::st_coordinates() |> as.numeric()
...
test_that("Fallbacks working as intended.", {
  ...
  expect_warning(get_centroid(c(20, 80), epsg = 4326))
})
