#' Interactive visualisation of tile locations from KOSTRA-DWD-2010R dataset
#'
  ...
  if (!is.null(x)) {
    ...
    m <- leaflet::leaflet() |>
      ...
      htmlwidgets::onRender("
        function() {
            $('.leaflet-control-layers-overlays').prepend('<label style=\"text-align:center\">KOSTRA-DWD-2010R</label>');
        }
    ")
    ...
    m <- leaflet::leaflet() |>
      ...
      leaflet::addScaleBar(position = "bottomleft",
                           options = leaflet::scaleBarOptions(imperial = FALSE)) |>
      leaflet::addLayersControl(baseGroups = c("CartoDB.Positron"),
                                overlayGroups = c("KOSTRA-DWD-2010R"))
  }
