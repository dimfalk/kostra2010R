...
#'
#' @examples
#' get_centroid(x = c(6.09, 50.46), epsg = 4326)
...
get_centroid <- function(x = NULL,
  ...
  # does the user input overlap with KOSTRA-DWD-2010R spatially?
  hits <- sf::st_intersects(kostra_dwd_2010r[[1]],
                            sf::st_transform(sf, "epsg:3034"),
                            sparse = FALSE) |> sum()

  if (hits == 0) {

    "Caution: Geocoded `x` does not intersect spatially with KOSTRA-DWD-2010R tiles." |> warning()
